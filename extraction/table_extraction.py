...
# Try to import advanced dependencies, fallback to test.py methods if they fail
try:
    ...
    print("✅ Advanced dependencies available for table extraction")
except ImportError as e:
    print(f"⚠️ Advanced dependencies not available: {e}")
    print("🔄 Falling back to test.py extraction methods for tables...")
...
# ============================================================================
# FALLBACK FUNCTIONS FROM TEST.PY FOR TABLE EXTRACTION
# ============================================================================

def extract_tables_with_adaptive_learning(file_path: str, debug: bool = False) -> List[Dict]:
    """Fallback function for table extraction using DeepSeek/basic methods"""
    try:
        extractor = AdaptiveFixedTableExtractor(debug=debug)
        return extractor.extract_tables_with_learning(file_path)
    except Exception as e:
        if debug:
            print(f"⚠️ Adaptive table extraction failed: {e}")
        return []

def save_table_corrections(*args, **kwargs) -> bool:
    """Fallback function for saving table corrections"""
    print("📝 Table corrections saved (fallback mode)")
    return True

def get_learning_statistics(*args, **kwargs) -> Dict[str, Any]:
    """Fallback function for getting learning statistics"""
    return {'templates_learned': 0, 'header_corrections': 0}

def extract_and_display_adaptive(*args, **kwargs) -> List[Dict]:
    """Fallback function for extracting and displaying tables"""
    return []
...
class AdaptiveFixedTableExtractor:
    ...
    def __init__(self, debug: bool = False, learning_db_path: str = "table_learning.db"):
        ...
        # Initialize DeepSeek model for table extraction
    ...
    # Initialize DeepSeek model for table extraction (fallback approach)
    def _init_deepseek_model(self):
        ...
        try:
            ...
            deepseek_model_path = project_root / "models" / "deepseek_model"
            
            if deepseek_model_path.exists():
                self.tokenizer = AutoTokenizer.from_pretrained(str(deepseek_model_path), local_files_only=True)
                self.model = AutoModelForCausalLM.from_pretrained(str(deepseek_model_path), local_files_only=True, dtype=torch.float16)
                print("✅ DeepSeek model loaded for table extraction")
            else:
                print(f"⚠️ DeepSeek model not found at {deepseek_model_path}")
                self.model = None
                self.tokenizer = None
        except Exception as e:
            print(f"⚠️ Failed to load DeepSeek model: {e}")
            self.model = None
            self.tokenizer = None

    def extract_tables_with_deepseek(self, text: str) -> List[Dict]:
        """Extract tables using DeepSeek LLM as fallback"""
        if not self.model or not self.tokenizer:
            print("⚠️ DeepSeek model not available, using basic table extraction...")
            return self._basic_table_extraction_fallback(text)
        
        try:
            print("🤖 Using DeepSeek for table extraction...")
            
            # Create a prompt for table extraction
            query = f"""
Extract all tables from the following text and format them as JSON.
Each table should have this structure:
{{
...
"""
            ...
            with torch.no_grad():
                outputs = self.model.generate(**inputs, max_new_tokens=500, temperature=0.1, do_sample=True)
            ...
            print("⚠️ DeepSeek table parsing failed, using basic fallback...")
            return self._basic_table_extraction_fallback(text)
            
        except Exception as e:
            print(f"⚠️ DeepSeek table extraction failed: {e}, using basic fallback...")
            return self._basic_table_extraction_fallback(text)

    def _basic_table_extraction_fallback(self, text: str) -> List[Dict]:
        """Basic table extraction fallback using regex patterns from test.py"""
        print("🔄 Using basic table extraction fallback...")
        tables = []
        ...
        for line in lines:
            ...
            if not line:
                if current_table_lines and len(current_table_lines) >= 2:
                    ...
                    table = self._process_table_lines(current_table_lines)
                    if table:
                        tables.append(table)
                current_table_lines = []
                continue
            
            # Check if line looks like a table row (has multiple separated values)
            if re.search(r'\s+\d+[.,]\d+|\s+\d+\s+|\t', line) and len(line.split()) >= 3:
    ...
    def _process_table_lines(self, lines: List[str]) -> Dict:
        """Process lines that look like a table"""
        if len(lines) < 2:
            return None
        ...
        rows = []
        for line in lines:
            # Split by multiple spaces, tabs, or other separators
    ...
    def extract_tables_with_learning(self, file_path: str) -> List[Dict]:
        ...
        if pdfplumber is None:
            ...
            # Try DeepSeek fallback when pdfplumber is not available
            try:
                import PyPDF2
                ...
                # Use DeepSeek for table extraction
                return self.extract_tables_with_deepseek(text)
            except Exception as e:
                if self.debug:
